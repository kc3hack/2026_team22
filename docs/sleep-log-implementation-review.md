...
1. **朝フローで「昨夜のログがない場合」に保存 UI を出す** ✅  
   - ホームで「朝の時間帯（起床時刻以降）」かつ「最新ログの date ≠ 昨日」のとき「昨夜を記録しましょう」カードを表示。  
   - タップで `AddSleepLogModal` を開き、昨日の日付（初期値）・スコア・ペナルティ・気分を入力して `addLog` を 1 回呼ぶ。409 時はモーダル内で「その日は既に記録があります」と表示。  
2. **モニター終了時にログを作る場合は**  
   - バックグラウンド or フォアグラウンドでモニターを止めたタイミングで、その日の date と集計結果を `addLog` に渡す。  
   - 既にその日のログがある場合は 409 になるので、そのときは「既に記録済み」と表示するか、何もしない。  
3. **「記録を追加」** ✅  
   - 睡眠ログ画面の履歴セクションに「＋ 記録を追加」ボタンを配置。タップで `AddSleepLogModal` を開き、日付（デフォルトは昨日）・スコア等を入力して `addLog`。409 時は「その日は既に記録があります。編集する場合は…」と表示。

このように「朝の確定」を主、「手動追加」を補助にした実装で、1日1ログ制約と整合している。
