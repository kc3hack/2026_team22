...
export const useAlarm = () => {
    ...
    const playSound = async (phase: 'gentle' | 'strict') => {
        try {
            ...
            const { sound } = await Audio.Sound.createAsync(
                source,
                { shouldPlay: true, isLooping: true, volume: initialVolume }
            );
    ...
    const getAdjustedWindow = async (alarmStartTime: number): Promise<number> => {
        ...
        if (todaysEvents.length > 0) {
            ...
            // Importance check
            const isImportant = firstEvent.title.includes('Important') || firstEvent.title.includes('Test');
            if (isImportant) {
    ...
    }, [alarmStore.isAlarmRinging, alarmStore.alarmStartTime, alarmStore.currentPhase, sleepSettings.resilienceWindowMinutes]);
